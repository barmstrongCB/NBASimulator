...
celticRPG <- otherV[5] + otherV[15] + otherV[25] + otherV[35] + otherV[45]
...
while(playCount < 225){
  ...
  if((ballGoes < allPlayerPct[1 + n]) & pendAssist != (1 + n)){
    ...
    print(allNames[1 + n])
    holdBall <- 1 + n
  }else if((ballGoes < (allPlayerPct[1 + n] + allPlayerPct[2 + n])) & pendAssist != (2 + n)) {
    addForPlayer <- 10
    holdBall <- 2 + n
    print(paste(allNames[holdBall],"has the ball"))
  }else if((ballGoes < (allPlayerPct[1 + n] + allPlayerPct[2 + n] + allPlayerPct[3 + n])) & pendAssist != (3 + n)) {
    addForPlayer <- 20
    holdBall <- 3 + n
    print(paste(allNames[holdBall],"has the ball"))
  }else if((ballGoes < (allPlayerPct[1 + n] + allPlayerPct[2 + n] + allPlayerPct[3 + n] + allPlayerPct[4 + n])) & pendAssist != (4 + n)) {
    addForPlayer <- 30
    holdBall <- 4 + n
    print(paste(allNames[holdBall],"has the ball"))
  }else {
...
if(noAssist & !playDone){
  ...
  if(noAssist){
    ...
    if(shootTwo){
      ...
      #first check block
      ranChk <- sample(1:100,1,replace=TRUE)
      if(ranChk < teamBlkV[dChk]) {
        #who blocks?
        ranChk <- sample(1:100,1,replace=TRUE) / 100
        if(ranChk < plyrBlkV[d + 1]){
          print(paste(allNames[6],"blocks the shot!"))
          holdBall <-1
        }else if(ranChk < plyrBlkV[d + 1] + plyrBlkV[d + 2]){
          print(paste(allNames[7],"blocks the shot!"))
          holdBall <-2
        }else if(ranChk < plyrBlkV[d + 1] + plyrBlkV[d + 2] + plyrBlkV[d + 3]){
          print(paste(allNames[8],"blocks the shot!"))
          holdBall <-3
        }else if(ranChk < (1 - plyrBlkV[d + 5])){
